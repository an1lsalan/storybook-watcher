...
async function handleNewComponent(componentPath, config) {
    ...
    if (fs.existsSync(storyFilePath)) {
        ...
        await handleChangeComponent(componentPath, config);
    ...
    const { extractedProps, typeChecker } = extractPropsFromComponent(componentPath, config.CWD, config.MAX_RECURSION_DEPTH, config.useStorybookActions); // config.useStorybookActions übergeben
    ...
    await formatFileWithPrettier(storyFilePath, config.CWD); // Prettier Aufruf HIER
    console.log(chalk.blueBright(`Datei formatiert mit Prettier: ${chalk.underline.white(path.relative(config.CWD, storyFilePath))}`));
...
async function handleChangeComponent(componentPath, config) {
    ...
    if (!fs.existsSync(storyFilePath)) {
        ...
        await handleNewComponent(componentPath, config);
        return;
    }

    const { extractedProps, typeChecker } = extractPropsFromComponent(componentPath, config.CWD, config.MAX_RECURSION_DEPTH, config.useStorybookActions); // config.useStorybookActions übergeben
    if (extractedProps && extractedProps.length > 0 && typeChecker) { // Stelle sicher, dass typeChecker existiert
        await updateStoryFile(storyFilePath, extractedProps, config.defaultStoryName, config.CWD, typeChecker, config);
    } else if (!typeChecker && extractedProps && extractedProps.length > 0) {
