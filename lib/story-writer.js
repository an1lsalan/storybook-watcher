...
function updateStoryFile(storyFilePath, extractedProps, defaultStoryName, CWD, typeChecker, config) {
                        ...
                        if (config.generateArgTypes) {
                            ...
                            if (argTypesProperty && argTypesProperty.value.type === 'ObjectExpression') {
                                ...
                                for (const prop of extractedProps) {
                                    ...
                                    if (typeInfo.options && typeInfo.controlString === "'select'") {
                                        const optionsArrayString = `[${typeInfo.options.join(', ')}]`;
                                        controlOutput = `control: { type: 'select', options: ${optionsArrayString} }`;
                                    } else if (typeInfo.options && typeInfo.controlString === "'radio'") {
                                        const optionsArrayString = `[${typeInfo.options.join(', ')}]`;
                                        controlOutput = `control: { type: 'radio', options: ${optionsArrayString} }`;
                                    }
